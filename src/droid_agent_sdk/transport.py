...
from .exceptions import FIFOError, TransportError
...
class FIFOTransport:
    ...
    @classmethod
    def restore(cls, fifo_path: str, log_path: str, pid: int | None = None) -> FIFOTransport:
...
def start_daemon(
    ...
    daemon_proc = subprocess.Popen(
        [
            sys.executable, "-m", "droid_agent_sdk.daemon_start",
            name, model, pr_number, cwd, auto_level,
        ],
...
def resume_daemon(
    ...
    daemon_proc = subprocess.Popen(
        [
            sys.executable, "-m", "droid_agent_sdk.daemon_resume",
            name, pr_number, session_id, cwd, auto_level,
        ],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
        start_new_session=True,
